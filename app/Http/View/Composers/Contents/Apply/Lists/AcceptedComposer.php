 ...
 * @author anhpd
 */
class AcceptedComposer extends BasicComposer implements ViewComposerInterface
{
    public function __construct(private ApplyListInterface $applyListQuery)
    {
    }
    ...
    public function createParameter(): array
    {
        ...
        $parameter1->setStatus([ApplyStatuses::ACCEPTED]);
        $parameter1->setIsRepliedBySecretariat(false);
        $parameter1->setIsShowAccepted(true);
        //パラメタ2
        $parameter2 = new ApplyListParameter();
        $parameter2->setStatus([ApplyStatuses::ACCEPTED]);
        $parameter2->setIsRepliedBySecretariat(true);
        $parameter2->setIsShowAccepted(true);
