...
use App\Http\Requests\Apply\Detail\SaveSection08Request;
use Illuminate\Support\Facades\Redirect;
use Ncc01\Apply\Application\Usecase\SaveApplySection08Interface;
...
/**
 * SaveSection08Controller
 *
 * @package App\Http\Controllers\Apply\Detail
 */
class SaveSection08Controller extends BaseSaveController
{
    /** @var SaveApplySection08Interface $saveApplySection08 */
    private $saveApplySection08;

    /**
     * SaveSection08Controller constructor.
     * @param ValidatePermissionModifyApplyInterface $validatePermissionModifyApply
     * @param SaveApplySection08Interface $saveApplySection08
    ...
    public function __construct(
        ...
        SaveApplySection08Interface $saveApplySection08,
        ...
        $this->saveApplySection08 = $saveApplySection08;
    }

    /**
     * __invoke
     *
     * @param SaveSection08Request $request
     * @param int $applyId
     * @return \Illuminate\Http\RedirectResponse
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function __invoke(SaveSection08Request $request, int $applyId)
    {
        //表示条件の確認
        $this->confirmConditions($applyId);

        //画面ロックのチェック
        if ($this->isSectionLocked($applyId, 'section8')) {
            abort(403, '画面がロックされているため、保存できません。');
        }

        //保存処理
        $this->saveApplySection08->__invoke($request->createSaveParameter(), $applyId);

        //メッセージ送信
        $this->sendMessageToSecretariat($request->createNotifyToSecretariatParameter($applyId, 8));

        return Redirect::route('apply.detail.section8', ['applyId' => $applyId]);
    }
}
