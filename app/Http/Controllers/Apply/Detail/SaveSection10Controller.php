...
use App\Http\Requests\Apply\Detail\SaveSection10Request;
...
/**
 * SaveSection10Controller
 *
 * @package App\Http\Controllers\Apply\Detail
 */
class SaveSection10Controller extends BaseSaveController
{
    /** @var SaveApplySection10Interface $saveApplySection10 */
    private $saveApplySection10;

    /**
     * SaveSection10Controller constructor.
     * @param ValidatePermissionModifyApplyInterface $validatePermissionModifyApply
     * @param SaveApplySection10Interface $saveApplySection10
    ...
    public function __construct(
        ...
        SaveApplySection10Interface $saveApplySection10,
        ...
        $this->saveApplySection10 = $saveApplySection10;
    }

    /**
     * __invoke
     *
     * @param SaveSection10Request $request
     * @param int $applyId
     * @return \Illuminate\Http\RedirectResponse
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function __invoke(SaveSection10Request $request, int $applyId)
    {
        //表示条件の確認
        $this->confirmConditions($applyId);

        //画面ロックのチェック
        if ($this->isSectionLocked($applyId, 'section10')) {
            abort(403, '画面がロックされているため、保存できません。');
        }

        //保存処理
        $this->saveApplySection10->__invoke($request->createSaveParameter(), $applyId);

        //メッセージ送信
        $this->sendMessageToSecretariat($request->createNotifyToSecretariatParameter($applyId, 10));

        return Redirect::route('apply.detail.section10', ['applyId' => $applyId]);
    }
}
