...
use App\Http\Requests\Apply\Detail\SaveSection09Request;
use Illuminate\Support\Facades\Redirect;
use Ncc01\Apply\Application\Usecase\SaveApplySection09Interface;
...
/**
 * SaveSection09Controller
 *
 * @package App\Http\Controllers\Apply\Detail
 */
class SaveSection09Controller extends BaseSaveController
{
    /** @var SaveApplySection09Interface $saveApplySection09 */
    private $saveApplySection09;

    /**
     * SaveSection09Controller constructor.
     * @param ValidatePermissionModifyApplyInterface $validatePermissionModifyApply
     * @param SaveApplySection09Interface $saveApplySection09
    ...
    public function __construct(
        ...
        SaveApplySection09Interface $saveApplySection09,
        ...
        $this->saveApplySection09 = $saveApplySection09;
    }

    /**
     * __invoke
     *
     * @param SaveSection09Request $request
     * @param int $applyId
     * @return \Illuminate\Http\RedirectResponse
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function __invoke(SaveSection09Request $request, int $applyId)
    {
        //表示条件の確認
        $this->confirmConditions($applyId);

        //画面ロックのチェック
        if ($this->isSectionLocked($applyId, 'section9')) {
            abort(403, '画面がロックされているため、保存できません。');
        }

        //保存処理
        $this->saveApplySection09->__invoke($request->createSaveParameter(), $applyId);

        //メッセージ送信
        $this->sendMessageToSecretariat($request->createNotifyToSecretariatParameter($applyId, 9));

        return Redirect::route('apply.detail.section9', ['applyId' => $applyId]);
    }
}
