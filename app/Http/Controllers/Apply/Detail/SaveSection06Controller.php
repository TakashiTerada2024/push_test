...
use App\Http\Requests\Apply\Detail\SaveSection06Request;
use Illuminate\Support\Facades\Redirect;
use Ncc01\Apply\Application\Usecase\SaveApplySection06Interface;
...
/**
 * SaveSection06Controller
 *
 * @package App\Http\Controllers\Apply\Detail
 */
class SaveSection06Controller extends BaseSaveController
{
    /** @var SaveApplySection06Interface $saveApplySection06 */
    private $saveApplySection06;

    /**
     * SaveSection06Controller constructor.
     * @param ValidatePermissionModifyApplyInterface $validatePermissionModifyApply
     * @param SaveApplySection06Interface $saveApplySection06
    ...
    public function __construct(
        ...
        SaveApplySection06Interface $saveApplySection06,
        ...
        $this->saveApplySection06 = $saveApplySection06;
    }

    /**
     * __invoke
     *
     * @param SaveSection06Request $request
     * @param int $applyId
     * @return \Illuminate\Http\RedirectResponse
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function __invoke(SaveSection06Request $request, int $applyId)
    {
        //表示条件の確認
        $this->confirmConditions($applyId);

        //画面ロックのチェック
        if ($this->isSectionLocked($applyId, 'section6')) {
            abort(403, '画面がロックされているため、保存できません。');
        }

        //保存処理
        $this->saveApplySection06->__invoke($request->createSaveParameter(), $applyId);

        //メッセージ送信
        $this->sendMessageToSecretariat($request->createNotifyToSecretariatParameter($applyId, 6));

        return Redirect::route('apply.detail.section6', ['applyId' => $applyId]);
    }
}
