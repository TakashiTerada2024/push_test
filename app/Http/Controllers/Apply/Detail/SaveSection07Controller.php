...
use App\Http\Requests\Apply\Detail\SaveSection07Request;
use Illuminate\Support\Facades\Redirect;
use Ncc01\Apply\Application\Usecase\SaveApplySection07Interface;
...
/**
 * SaveSection07Controller
 *
 * @package App\Http\Controllers\Apply\Detail
 */
class SaveSection07Controller extends BaseSaveController
{
    /** @var SaveApplySection07Interface $saveApplySection07 */
    private $saveApplySection07;

    /**
     * SaveSection07Controller constructor.
     * @param ValidatePermissionModifyApplyInterface $validatePermissionModifyApply
     * @param SaveApplySection07Interface $saveApplySection07
    ...
    public function __construct(
        ...
        SaveApplySection07Interface $saveApplySection07,
        ...
        $this->saveApplySection07 = $saveApplySection07;
    }

    /**
     * __invoke
     *
     * @param SaveSection07Request $request
     * @param int $applyId
     * @return \Illuminate\Http\RedirectResponse
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function __invoke(SaveSection07Request $request, int $applyId)
    {
        //表示条件の確認
        $this->confirmConditions($applyId);

        //画面ロックのチェック
        if ($this->isSectionLocked($applyId, 'section7')) {
            abort(403, '画面がロックされているため、保存できません。');
        }

        //保存処理
        $this->saveApplySection07->__invoke($request->createSaveParameter(), $applyId);

        //メッセージ送信
        $this->sendMessageToSecretariat($request->createNotifyToSecretariatParameter($applyId, 7));

        return Redirect::route('apply.detail.section7', ['applyId' => $applyId]);
    }
}
