...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section04;
...
class UpdateSection04Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section04_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection04($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // Act
        $response = $this->post("/apply/detail/section4/{$apply->id}", [
            '4_year_of_diagnose_start' => 2020,
            '4_year_of_diagnose_end' => 2023,
            '4_area_prefectures' => [13, 14], // 東京都、神奈川県
            '4_idc_type' => 1,
            '4_idc_detail' => 'テスト詳細',
            ...
            '4_sex' => 1,
            '4_sex_detail' => 'テスト性別詳細',
            '4_range_of_age_type' => 1,
            '4_range_of_age_detail' => 'テスト年齢範囲詳細'
        ]);

        // Assert
        $response->assertStatus(302)
                ->assertRedirect(route('apply.detail.section4', ['applyId' => $apply->id]));
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section04_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection04(int $applyId, bool $locked = true): void
    {
        if ($locked === null) {
            return;
        }
