...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section07;
...
class UpdateSection07Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section07_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection07($apply->id, $isLocked);
        }
        ...
        // テスト用のファイルを作成
        $files = [
            UploadedFile::fake()->create('test701_1.pdf', 100),
            UploadedFile::fake()->create('test701_2.pdf', 100)
        ];

        // Act
        $response = $this->post("/apply/detail/section7/{$apply->id}", [
            'attachments701' => $files,
            'active_attachements701' => []  // 既存のファイルIDの配列（今回は新規なので空）
        ]);

        // Assert
        $response->assertStatus(302)
                ->assertRedirect(route('apply.detail.section7', ['applyId' => $apply->id]));
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section07_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection07(int $applyId, bool $locked = true): void
    {
        if ($locked === null) {
            return;
        }
