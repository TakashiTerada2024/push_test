...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section05;
...
class UpdateSection05Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section05_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection05($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // テスト用のファイルを作成
        $file501 = UploadedFile::fake()->create('test501.pdf', 100);
        $file502 = UploadedFile::fake()->create('test502.pdf', 100);

        // Act
        $response = $this->post("/apply/detail/section5/{$apply->id}", [
            '5_research_method' => 'テスト研究方法の説明',
            'attachment501' => $file501,
            'attachment502' => $file502
        ]);

        // Assert
        $response->assertStatus(302)
                ->assertRedirect(route('apply.detail.section5', ['applyId' => $apply->id]));
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section05_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection05(int $applyId, bool $locked = true): void
    {
        if ($locked === null) {
            return;
        }
