...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section09;
...
class UpdateSection09Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section09_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection09($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // Act
        $response = $this->post("/apply/detail/section9/{$apply->id}", [
            '9_treatment_after_use' => '研究終了後、速やかにデータを削除します。'
        ]);

        // Assert
        $response->assertStatus(302)
                ->assertRedirect(route('apply.detail.section9', ['applyId' => $apply->id]));
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section09_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection09(int $applyId, bool $locked = true): void
    {
        if ($locked === null) {
            return;
        }
