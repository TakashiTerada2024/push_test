...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section01;
...
class UpdateSection01Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section01_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        $apply = Apply::factory()->create([
            'status' => $status,
            'user_id' => $user->id,
            'type_id' => ApplyTypes::GOVERNMENT_LINKAGE
        ]);

        // ロック状態を設定（nullの場合は何もしない）
        if ($isLocked !== null) {
            $this->lockSection01($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // Act
        $response = $this->post("/apply/detail/section1/{$apply->id}", [
            'dummy_data' => 'test'  // 実際のフィールド名は関係ないため、ダミーデータを送信
        ]);

        // Assert
        $response->assertStatus(302)  // リダイレクトステータスコード
                ->assertRedirect(route('apply.detail.section1', ['applyId' => $apply->id]));  // リダイレクト先の確認
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section01_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection01(int $applyId, bool $locked = true): void
    {
