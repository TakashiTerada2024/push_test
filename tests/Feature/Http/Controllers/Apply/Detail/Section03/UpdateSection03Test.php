...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section03;
...
class UpdateSection03Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section03_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection03($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // Act
        $response = $this->post("/apply/detail/section3/{$apply->id}", [
            '3_number_of_users' => 1,
            'apply_users' => [
                [
                    'name' => 'テストユーザー',
            ...
            '10_applicant_type' => '1',
            '10_applicant_name_1' => 'テスト申請者',
            '10_applicant_address_1' => 'テスト住所',
            '10_applicant_birthday_1' => '1990-01-01',
            'affiliation_1' => 'テスト所属'
        ]);

        // Assert
        $response->assertStatus(302)  // リダイレクトステータスコード
                ->assertRedirect(route('apply.detail.section3', ['applyId' => $apply->id]));  // リダイレクト先の確認
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section03_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection03(int $applyId, bool $locked = true): void
    {
