...
namespace Tests\Feature\Http\Controllers\Apply\Detail\Section08;
...
class UpdateSection08Test extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_can_update_section08_when_conditions_are_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        ...
        if ($isLocked !== null) {
            $this->lockSection08($apply->id, $isLocked);
        }

        $this->actingAs($user);

        // Act
        $response = $this->post("/apply/detail/section8/{$apply->id}", [
            '8_scheduled_to_be_announced' => '2024年12月頃に発表予定'
        ]);

        // Assert
        $response->assertStatus(302)
                ->assertRedirect(route('apply.detail.section8', ['applyId' => $apply->id]));
    }

    /**
     * @dataProvider failedUpdateDataProvider
     */
    public function test_cannot_update_section08_when_conditions_are_not_met(string $userRole, ?bool $isLocked, int $status)
    {
        // Arrange
        $user = User::factory()->create([
            'role_id' => $userRole === 'secretary' ? self::ROLE_SECRETARY : self::ROLE_APPLICANT
    ...
    private function lockSection08(int $applyId, bool $locked = true): void
    {
        if ($locked === null) {
            return;
        }
