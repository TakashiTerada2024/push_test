...
/**
 * Class OtherApplicantCanNotModifyApplyAfterCheckingDocumentStatusTest
 * @package Tests\Feature\Http\Controllers\Apply\Detail
 */
class OtherApplicantCanNotModifyApplyTest extends FeatureTestBase
{
    /**
     * ステータスに関わらず、所有者でない申請編集画面に対してPOSTが許可されないことを検証する。
    ...
    public function test($url, $postParameters)
    {
        ...
        foreach ($statuses as $applyStatusId => $name) {
            ...
            //テスト実行
            $response = $this->actingAs($otherApplicant)->post($path, $postParameters);
            //assertions
