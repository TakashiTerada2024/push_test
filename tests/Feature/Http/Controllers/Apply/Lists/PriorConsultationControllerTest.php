...
/**
 * Class PriorConsultationControllerTest
 * コントローラーに対する正常表示、権限の確認テスト
 * @package Tests\Feature\Http\Controllers\Apply\Lists
 */
class PriorConsultationControllerTest extends FeatureTestBase
{
    /**
     * test_Applicant
     *
     * @covers \App\Http\Controllers\Apply\Lists\PriorConsultationController
     * @author kenji yamamoto <k.yamamoto@balocco.info>
     */
    public function test_ApplicantUserCanNotSeeList()
    {
        $applicantActor = User::find(101);
        $response = $this->actingAs($applicantActor)->get('/apply/lists/prior_consultation');
    ...
    public function test_SecretariatUserCanSeeList()
    {
        ...
        $response = $this->actingAs($secretariatActor)->get('/apply/lists/prior_consultation');
