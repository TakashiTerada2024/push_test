...
/**
 *  TreatmentAfterUseTest
 *
 * PDF出力内容テスト(9.情報等の利用後の処置)
 *
 * @package Http\Controllers\Pdf
 */
class TreatmentAfterUseTest extends FeatureTestBase
{
     ...
     * @dataProvider nDataProvider_情報等の利用後の処置を出力
     */
    public function test_download_情報等の利用後の処置を出力(
        int $actorIs,
        int $applyStatusId,
        ?int $applyType,
        ?string $expectText
        ...
        $targetApply = Apply::factory()->state([
            ...
            'type_id' => $applyType,
            '9_treatment_after_use' => $expectText
        ])->create();
        ...
        // Assertion
        // 6ページ目の情報等の利用後の処置が一致すること
        $this->assertMatchesRegularExpression(sprintf('/%s/', $expectText), $targetTextList[4]);
    }

    /**
     * nDataProvider_情報等の利用後の処置を出力
     *
     * @return array
     * @author m.shomura <m.shomura@balocco.info>
     */
    public function nDataProvider_情報等の利用後の処置を出力()
    {
        return [
            // 申請者本人
            "申請者本人_行政関係者・リンケージ利用" => [self::ACTOR_IS_OWNER, 2, ApplyTypes::GOVERNMENT_LINKAGE, '利用後の処置テスト改行なし1'],
            "申請者本人_行政関係者・集計統計利用" => [self::ACTOR_IS_OWNER, 3, ApplyTypes::GOVERNMENT_STATISTICS, '利用後の処置テスト改行なし2'],
            "申請者本人_研究者等・リンケージ利用" => [self::ACTOR_IS_OWNER, 4, ApplyTypes::CIVILIAN_LINKAGE, '利用後の処置テスト改行なし3'],
            "申請者本人_研究者等・集計統計利用" => [self::ACTOR_IS_OWNER, 2, ApplyTypes::CIVILIAN_STATISTICS, '利用後の処置テスト改行なし4'],

            // 事務局
            "事務局_行政関係者・リンケージ利用" => [self::ACTOR_IS_SECRETARIAT, 3, ApplyTypes::GOVERNMENT_LINKAGE, '利用後の処置テスト改行なし5'],
            "事務局_行政関係者・集計統計利用" => [self::ACTOR_IS_SECRETARIAT, 4, ApplyTypes::GOVERNMENT_STATISTICS, '利用後の処置テスト改行なし6'],
            "事務局_研究者等・リンケージ利用" => [self::ACTOR_IS_SECRETARIAT, 20, ApplyTypes::CIVILIAN_LINKAGE, '利用後の処置テスト改行なし7'],
            "事務局_研究者等・集計統計利用" => [self::ACTOR_IS_SECRETARIAT, 3, ApplyTypes::CIVILIAN_STATISTICS, null],
        ];
    }

    /**
     * test_download_情報等の利用後の処置を出力_改行あり
     *
     * @param int $actorIs
     * @param int $applyStatusId
     * @param int|null $applyType
     ...
     * @author m.shomura <m.shomura@balocco.info>
     * @dataProvider nDataProvider_情報等の利用後の処置を出力_改行あり
     */
    public function test_download_情報等の利用後の処置を出力_改行あり(
        int $actorIs,
        int $applyStatusId,
        ?int $applyType,
        ...
        $targetApply = Apply::factory()->state([
            ...
            'status' => $applyStatusId,
            'type_id' => $applyType,
            '9_treatment_after_use' => $scheduledToBeAnnounced
        ])->create();
        ...
        // Assertion
        // 6ページ目の情報等の利用後の処置に改行コードがあること
        $this->assertMatchesRegularExpression(sprintf('/%s<br/', $expectText), $targetTextList[4]);
    }

    /**
     * nDataProvider_情報等の利用後の処置を出力_改行あり
     *
     * @return array
     * @author m.shomura <m.shomura@balocco.info>
     */
    public function nDataProvider_情報等の利用後の処置を出力_改行あり()
    {
        $text = <<<EOT
        利用後の処置テスト%s
        改行テスト
        EOT;
        $exceptText = "利用後の処置テスト%s";
        return [
            // 申請者本人
