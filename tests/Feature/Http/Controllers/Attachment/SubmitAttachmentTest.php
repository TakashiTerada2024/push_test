...
/**
 * Class SubmitAttachmentTest
 * 添付ファイル提出に関する機能テスト
 *
 * @package Tests\Feature\Http\Controllers\Attachment
 */
class SubmitAttachmentTest extends FeatureTestBase
{
    /**
     * test_ownerSubmitAttachment
     *
     ...
     */
    public function test_ownerSubmitAttachment(int $attachmentStatus, int $expectedStatus)
    {
        // ユーザー作成
        // テスト用申出者の作成
    ...
    {
        return [
            "申出者_アップロードの場合302" => [AttachmentStatuses::UPLOADED, 302],
            "申出者_提出済の場合403" => [AttachmentStatuses::SUBMITTING, 403],
            "申出者_承認済の場合403" => [AttachmentStatuses::APPROVED, 403],
        ];
    }

    /**
     * test_otherApplicantCannotSubmitAttachment
     *
     * @param int $attachmentStatus
     * @author m.shomura <m.shomura@balocco.info>
     * @dataProvider attachmentStatusDataProvider
     */
    public function test_otherApplicantCannotSubmitAttachment(int $attachmentStatus)
    {
        // ユーザー作成
        // テスト用申出者の作成
    ...
    /**
     * test_adminSubmitAttachment
     *
     * @param int $attachmentStatus
     * @param int $expectedStatus
     * @param int $roleId
     * @author m.shomura <m.shomura@balocco.info>
     * @dataProvider adminApplyStatusDataProvider
     */
    public function test_adminSubmitAttachment(int $attachmentStatus, int $expectedStatus, int $roleId)
    {
        // ユーザー作成
        // テスト用申出者の作成
    ...
    /**
     * adminApplyStatusDataProvider
     *
     * @return array
     */
    public function adminApplyStatusDataProvider(): array
    {
        return [
            "管理者_アップロードの場合403" => [AttachmentStatuses::UPLOADED, 403, 1],
            "管理者_提出済の場合403" => [AttachmentStatuses::SUBMITTING, 403, 1],
            "管理者_承認済の場合403" => [AttachmentStatuses::APPROVED, 403, 1],
            "事務局_アップロードの場合403" => [AttachmentStatuses::UPLOADED, 403, 2],
            "事務局_提出済の場合403" => [AttachmentStatuses::SUBMITTING, 403, 2],
