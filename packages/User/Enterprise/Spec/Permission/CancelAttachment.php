...
/**
 * CancelAttachment
 * 添付ファイルに対する提出キャンセルを行う権限があるかを判定する。
 * 申出情報(apply)、添付ファイル情報(attachment)の二段があるため少し複雑。
 * @package Ncc01\User\Enterprise\Spec\Permission
 */
class CancelAttachment implements PermissionSpecInterface
{
    /**
     * __construct
     *
    ...
    /**
     * canCancelStatus
     * 「提出済」ステータスのみ「キャンセル」可能
     *
     * @return bool
     */
    protected function canCancelStatus(): bool
    {
        if ($this->attachment->getStatus() == AttachmentStatuses::SUBMITTING) {
            return true;
        }
        return false;
    }
