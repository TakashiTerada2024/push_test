...
use Ncc01\User\Enterprise\Spec\Permission\ShowInformationAboutApply;
 ...
 */
class ValidateShowApplyParameter implements ValidatePermissionParameterInterface
{
    ...
    {
        $baseInfo = $this->retrieveApplyBaseInfo->__invoke($this->applyId);

        return new ShowInformationAboutApply(
            $loginUser->getRole(),
            $loginUser->getId(),
            $baseInfo->getApplicantUserId(),
            $baseInfo->getStatusId()
        );
    }
