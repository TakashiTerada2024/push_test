...
/**
 * YearOfDiagnoseEndSpec
 *
 * @package Ncc01\Apply\Enterprise\Spec\CheckingDocument2\Items\Section04
 */
class YearOfDiagnoseEndSpec extends ValidationSpec
{
    private YearsOfDiagnose $yearsOfDiagnoseList;

    public function __construct(mixed $candidate, private ?int $yearOfDiagnoseStart)
    {
    ...
    public function definition(): mixed
    {
        ...
        if ($this->getCandidate() && $this->yearOfDiagnoseStart) {
            //開始年次が決定している場合は、開始年次以上の数値であることを検証
            $validator->oneOf(
                Validator::equivalent($this->yearOfDiagnoseStart),
                Validator::greaterThan($this->yearOfDiagnoseStart)
            );
        }
        return $validator;
    }
    ...
    {
        return '4_year_of_diagnose_end';
    }

    public function getSpecName(): string
    {
        return (new QuestionItemName())->value('4_year_of_diagnose') . '（終期）';
    }
}
