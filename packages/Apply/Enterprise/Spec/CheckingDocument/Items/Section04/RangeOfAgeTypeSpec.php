...
use Ncc01\Apply\Enterprise\Classification\RangesOfAge;
 ...
 */
class RangeOfAgeTypeSpec extends ValidationSpec
{
    private ?ApplyType $applyType;

    public function __construct(mixed $candidate, ?ApplyType $applyType)
    {
    ...
    {
        if ($this->isLinkageType()) {
            return Validator::alwaysValid();
        }
        return Validator::notEmpty();
    }

    private function isLinkageType(): bool
    {
        return $this->applyType?->isLinkage() ?? false;
    }

    public function getSpecKey(): string
    {
        return '4_range_of_age_type';
    }

    public function getSpecName(): string
    {
        return (new QuestionItemName())->value('4_range_of_age_type');
    }
}
