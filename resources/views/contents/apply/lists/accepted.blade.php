    ...
    <x-slot name="header">
        ...
        <div class="block mb-4">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ config('app-ncc01.system.title') }} 提出中 一覧
            </h2>
        </div>
        ...
        <a href="{{route('apply.lists.accepted')}}"><x-button-tab class="bg-gray-100">応諾</x-button-tab></a>
        ...
        <table class="table-fixed w-full bg-white">
            ...
            @foreach($appliesNotReplied as $row)
                <tr class="border">
                    ...
                    <td rowspan="2" class="p-2 text-center">
                        ...
                        @else
                            @if($applyStatuses->whetherToShowApplyDetail($row->status,$row->type_id))
                                {{ $row->id }}<br />
                                <span style="font-size: smaller;">-><a href="{{route('apply.detail.overview',['applyId'=>$row->apply_id])}}" class="link-primary">{{ $row->apply_id }}<span /></a>
                            @else
                                {{ $row->id }}<br />
                                <span style="font-size: smaller;">-> <a href="#">{{ $row->apply_id }}</a></span>
                            @endif
                        @endif
                    </td>
                    ...
                    <td rowspan="2" class="text-center"><x-label-status :status-id="$row->status" :destination-apply-id="$row->apply_id ?? 0"/></td>
